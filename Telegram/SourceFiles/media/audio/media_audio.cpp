...
int32 Fader::updateOnePlayback(Mixer::Track *track, bool &hasPlaying, bool &hasFading, float64 volumeMultiplier, bool volumeChanged) {
	...
	if (alState != AL_PLAYING && !track->loading) {
		...
		if (Core::App().settings().skipSilenceEnabled() && track->state.id.type() == AudioMsgId::Type::Video) {
			// Find upcoming samples RMS
			float64 rms = 0.0;
			int channels = (track->format == AL_FORMAT_STEREO16 || track->format == AL_FORMAT_STEREO8) ? 2 : 1;
			int bytesPerSample = (track->format == AL_FORMAT_STEREO16 || track->format == AL_FORMAT_MONO16) ? 2 : 1;
			int samplesToCheck = std::min(track->state.frequency / 10, 4096);
			int numSamplesChecked = 0;
			int currentOffset = positionInBuffered;
			int64 sumSquares = 0;

			for (int bufId = 0; bufId < Mixer::Track::kBuffersCount; ++bufId) {
				if (!track->withSpeed.samples[bufId] || track->withSpeed.buffered[bufId].isEmpty()) continue;

				int bufferSamples = track->withSpeed.samples[bufId];
				if (currentOffset >= bufferSamples) {
					currentOffset -= bufferSamples;
					continue;
				}

				const char* data = track->withSpeed.buffered[bufId].constData();
				int remainingInThisBuf = bufferSamples - currentOffset;
				int toCheckHere = std::min(samplesToCheck - numSamplesChecked, remainingInThisBuf);

				if (bytesPerSample == 2) {
					const int16* ptr = reinterpret_cast<const int16*>(data) + currentOffset * channels;
					for (int k = 0; k < toCheckHere * channels; ++k) {
						int16 val = ptr[k];
						sumSquares += int64(val) * val;
					}
					numSamplesChecked += toCheckHere;
				} else if (bytesPerSample == 1) {
					const uint8* ptr = reinterpret_cast<const uint8*>(data) + currentOffset * channels;
					for (int k = 0; k < toCheckHere * channels; ++k) {
						int16 val = (int16(ptr[k]) - 128) * 256;
						sumSquares += int64(val) * val;
					}
					numSamplesChecked += toCheckHere;
				}
				...
				currentOffset = 0;
				if (numSamplesChecked >= samplesToCheck) break;
			}

			if (numSamplesChecked > 0) {
				rms = std::sqrt(float64(sumSquares) / float64(numSamplesChecked * channels)) / 32768.0;
				float pitch = (rms < 0.02) ? 4.0f : 1.0f;

				ALfloat currentPitch = 1.0f;
				alGetSourcef(track->stream.source, AL_PITCH, &currentPitch);
				if (currentPitch != pitch) {
					alSourcef(track->stream.source, AL_PITCH, pitch);
				}
			}
		} else {
			ALfloat currentPitch = 1.0f;
			...
			if (currentPitch != 1.0f) {
				alSourcef(track->stream.source, AL_PITCH, 1.0f);
			}
		}
	}
	track->withSpeed.fineTunedPosition = withSpeedPosition;
