...
void FillSpeedMenu(
	...
	auto toggleAction = base::make_unique_q<Ui::Menu::Action>(
		menu,
		st.dropdown.menu,
		Ui::Menu::CreateAction(menu, QString("Skip Silence"), [=] {
			const auto current = Core::App().settings().skipSilenceEnabled();
			Core::App().settings().setSkipSilenceEnabled(!current);
		}),
		&st.skipSilence,
		&st.skipSilence);
	const auto toggleRaw = toggleAction.get();
	const auto toggleCheck = Ui::CreateChild<Ui::RpWidget>(toggleRaw);
	toggleCheck->resize(st.activeCheck.size());
	toggleCheck->paintRequest(
	) | rpl::on_next([toggleCheck, icon = &st.activeCheck] {
		auto p = QPainter(toggleCheck);
		icon->paint(p, 0, 0, toggleCheck->width());
	}, toggleCheck->lifetime());
	toggleRaw->sizeValue(
	) | rpl::on_next([=, skip = st.activeCheckSkip](QSize size) {
		toggleCheck->moveToRight(
			skip,
			(size.height() - toggleCheck->height()) / 2,
			size.width());
	}, toggleCheck->lifetime());
	toggleCheck->setAttribute(Qt::WA_TransparentForMouseEvents);
	
	// We use a dummy timer to regularly poll the setting state so the checkbox updates
	// or we can just rely on the toggle directly. 
	// The best way for UI reactivity is watching the settings event stream. But we don't have
	// an event stream for this yet. We'll add one in Core::Settings, or just update on click.
	// For now let's just make it toggle via click and update visibility.
	toggleCheck->setVisible(Core::App().settings().skipSilenceEnabled());
	menu->addAction(std::move(toggleAction));
}
