...
QByteArray Settings::serialize() const {
	...
	size += sizeof(qint32) * 2
		...
		+ sizeof(qint32)
		+ sizeof(qint32); // _skipSilenceEnabled
	...
	result.reserve(size);
	{
		...
		stream
			...
			<< qint32(_skipSilenceEnabled ? 1 : 0);
	}
...
void Settings::addFromSerialized(const QByteArray &serialized) {
	...
	auto skipSilenceEnabled = qint32(0);
	if (!stream.atEnd()) {
		stream >> skipSilenceEnabled;
	}
	if (stream.status() != QDataStream::Ok) {
		LOG(("App Error: "
	...
	_skipSilenceEnabled = (skipSilenceEnabled == 1);
}
